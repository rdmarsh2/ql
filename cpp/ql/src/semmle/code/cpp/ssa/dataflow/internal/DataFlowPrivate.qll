...
class ArgumentNode extends Node {
  ArgumentNode() {
    exists(InvokeInstruction inv, PositionalArgumentOperand argTag |
      this = inv.getOperand(argTag)
    ) or exists(InvokeInstruction inv, ThisArgumentOperand thisArgTag |
      this = inv.getOperand(thisArgTag)
    )
  }
  ...
  predicate argumentOf(Call call, int pos) {
    exists (InvokeInstruction inv |
      inv.getAST() = call and
      (
        exists(PositionalArgumentOperand argTag |
          this = inv.getOperand(argTag) and
          argTag.getArgIndex() = pos
        ) or (
          pos = -1 and
          exists(ThisArgumentOperand argTag |
            this = inv.getOperand(argTag)
          )
        )
      )
...
class ReturnNode extends Node {
  ReturnNode() {
    exists(ReturnValueInstruction ret | this = ret.getAnOperand() )
  }
}
